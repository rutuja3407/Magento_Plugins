...
class IdentityProviders
{
    ...
    public function __construct(DOMElement $xml = NULL)
    {
        ...
        $idpSSODescriptor = SAML2Utilities::xpQuery($xml, './saml_metadata:IDPSSODescriptor');
        ...
        $info = SAML2Utilities::xpQuery($xml, './saml_metadata:Extensions');
    ...
    {
        $displayNames = SAML2Utilities::xpQuery($xml, './mdui:UIInfo/mdui:DisplayName');
    ...
    {
        $ssoServices = SAML2Utilities::xpQuery($xml, './saml_metadata:SingleSignOnService');
    ...
    {
        $sloServices = SAML2Utilities::xpQuery($xml, './saml_metadata:SingleLogoutService');
    ...
    {
        foreach (SAML2Utilities::xpQuery($xml, './saml_metadata:KeyDescriptor') as $KeyDescriptorNode) {
    ...
    {
        $certNode = SAML2Utilities::xpQuery($xml, './ds:KeyInfo/ds:X509Data/ds:X509Certificate');
    ...
    private function parseSigningCertificate($xml)
    {
        ...
        $certData = str_replace(array("\r", "\n", "\t", ' '), '', $certData);
        if (!empty($certNode))
            array_push($this->signingCertificate, SAML2Utilities::sanitize_certificate($certData));
    }
